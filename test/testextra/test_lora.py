...
class TestLoRALinear(unittest.TestCase):
  ...
  def test_from_linear_preserves_weight(self):
    ...
    w, b = linear.weight.numpy(), linear.bias.numpy()
    lora = LoRALinear.from_linear(linear, r=R)
    np.testing.assert_allclose(lora.weight.numpy(), w, atol=0)
    np.testing.assert_allclose(lora.bias.numpy(), b, atol=0)
...
class TestApplyLoRA(unittest.TestCase):
  def test_apply_to_model(self):
    class Model:
      def __init__(self):
        self.f1, self.f2 = Linear(IN_DIM, OUT_DIM), Linear(OUT_DIM, IN_DIM)
      def __call__(self, x): return self.f2(self.f1(x).relu())
    model = Model(); apply_lora(model, r=R, alpha=16)
    self.assertIsInstance(model.f1, LoRALinear)
    self.assertIsInstance(model.f2, LoRALinear)
  ...
  def test_apply_with_filter(self):
    class Model:
      ...
      def __call__(self, x): return self.o(self.qkv(x)) + self.ffn(x)
    model = Model()
    apply_lora(model, r=R, alpha=16, module_filter_fn=lambda _, fqn: "qkv" in fqn or fqn == "o")
  ...
  def test_apply_to_nested_model(self):
    class Block:
      def __init__(self): self.proj = Linear(IN_DIM, OUT_DIM)
    class Model:
      def __init__(self):
  ...
  def test_apply_preserves_forward(self):
    class Model:
      def __init__(self): self.fc = Linear(IN_DIM, OUT_DIM)
      def __call__(self, x): return self.fc(x)
    model, x = Model(), Tensor.randn(BS, IN_DIM)
...
class TestLoRAOptimizer(unittest.TestCase):
  ...
  def test_get_lora_params_are_trainable(self):
    class Model:
      def __init__(self): self.f = Linear(IN_DIM, OUT_DIM)
    model = Model(); apply_lora(model, r=R, alpha=16)
  ...
  def test_optimizer_only_updates_lora(self):
    ...
    w_before, b_before = model.f1.weight.numpy(), model.f1.lora_B.numpy()
    with Context(TRAINING=1):
      loss = (model(x) - y).abs().mean(); loss.backward()
      optimizer.step(); optimizer.zero_grad()
    np.testing.assert_allclose(model.f1.weight.numpy(), w_before, atol=0, err_msg="base weight should not change")
    self.assertFalse(np.allclose(model.f1.lora_B.numpy(), b_before, atol=1e-6), "lora_B should change")
  ...
  def test_optimizer_with_all_params_skips_frozen(self):
    class Model:
      ...
      def __call__(self, x): return self.fc(x)
    model = Model(); apply_lora(model, r=R, alpha=16)
    optimizer = optim.SGD(get_parameters(model), lr=1e-2)
    self.assertEqual(len(optimizer.params), 2)
    self.assertEqual(len(optimizer.buffers), 2)
